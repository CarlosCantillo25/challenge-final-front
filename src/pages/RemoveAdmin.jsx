...
function GrantPermissions() {
  ...
  const handleAdminToggle = async () => {
    try {
      const updatedUser = { ...users.find((user) => user._id === selectedUserId)};

      const response = await api.delete(apiUrl + endpoints.delete + selectedUserId, updatedUser );
  ...
  return (
    <div className='flex flex-col w-full justify-around items-center p-4 h-[60vh]'>
      <h2 className='font-semibold text-lg mb-4'>REMOVE ADMINISTRATOR PERMISSIONS</h2>
      <div className='flex flex-col justify-around items-center'>
        ...
        <div className='flex'>
          ...
          {selectedUserId && (
            <div className='m-4'>
              <button onClick={handleAdminToggle} className={`p-2 border-2 font-semibold rounded-xl bg-red-400`}>
                Delete
              </button>
            </div>
          )}
